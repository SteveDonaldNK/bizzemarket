...
export default function LoginDialog(props) {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    const payload = [...formData];
    try {
      axios.post("http://localhost:4000/register", {userName: payload[0][1], email: payload[1][1], password: payload[2][1]}, {
        headers:{
          'Content-Type':'application/json'
      },
      withCredentials: true
    ...
    <>
      ...
      <List sx={{ padding: "20px"}}>
        {LoginMethod.map((method) => (
          <a style={{textDecoration: "none", color: "#000"}} href={`http://localhost:4000/auth/${method.account}`}>
