...
export default function ProductDetails({ similarProducts, currentProduct, sellerData}) {
  ...
  const handleChoice = () => {
    if (choice !== '') {
        try {
            axios.patch("/api/pending", {choice: choice, productId: currentProduct._id}, {
                headers: {
  ...
  const increment = () => {
    try {
        axios.get(`/api/incrementView/${currentProduct._id}`);
    } catch (err) {
        ...
        <Grid justifyContent="space-between" container>
            <Grid sx={{height: "min-content"}} item xs={12} sm={12} md={7} lg={7}>
                <Box className={classes.box}>
                    <img className={classes.mainImg} src={`/api/images/${currentImage}`} alt={currentProduct.title} />
                </Box>
                <Grid spacing={1} sx={{mt: 0, mb: 2}} container>
                    {currentProduct.img.map((link) => (
                        <Grid onClick={() => handleImageClick(link)} sx={imgSize} item xs={4} sm={4} md={4} lg={4}>
                            <img className={classes.img} src={`/api/images/${link}`} alt={link.title} />
                        </Grid>
                    ))}
                </Grid>
