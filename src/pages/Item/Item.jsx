...
export default function Item() {
  ...
  const fetchProduct = async () => {
    try {
        const {data} = await axios.get(`http://54.197.36.149/api/products/${productCategory}/${productID}`);
...
const fetchSimilarProducts = async () => {
  try {
    let {data} = await axios.get(`http://54.197.36.149/api/products/${productCategory}`);
    data = data.slice(0, 6);
...
const fetchSeller= async () => {
  try {
    axios.get(`http://54.197.36.149/api/seller/${productID}`)
    .then((res) => {
